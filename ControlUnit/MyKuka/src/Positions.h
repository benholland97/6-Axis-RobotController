...
public:
	...
	bool operator==(const MyPoint& b) {
		return x!=b.x? false:y!=b.y? false:z!=b.z? false:true;
	}
...
public:
	...
	bool operator==(const JointAngles& b) {
		for(int i=0;i<NO_ACTUATORS;++i){
	...
	void toggleGripper() {
		a[NO_ACTUATORS-1] = a[NO_ACTUATORS-1] == 0? toRadians(1):toRadians(0);
	}
...
public:
    ...
    FullPosition(double p1, double p2, double p3, double r1, double r2, double r3) {
        null();
    	position = MyPoint(p1,p2,p3);
    	orientation = Rotation(r1,r2,r3);
    }
    ...
    void closeGripper(){
    	if(angles[NO_ACTUATORS-1] == 0) angles.toggleGripper();
    }
